...
def statistical_parity(predictions, protected):
    ...
    # 1 - |P(\hat{Y}=1|X_s=1) - P(\hat{Y}=1|X_s=0)|
...
def equalized_odds(labels, predictions, protected):
    ...
    # 2 - |P(\hat{Y}=1|Y=1,X_s=0)-P(\hat{Y}=1|Y=1,X_s=1)| - |P(\hat{Y}=1|Y=0,X_s=0)-P(\hat{Y}=1|Y=0,X_s=1)|
...
def equality_of_opportunity(labels, predictions, protected):
    ...
    # 1 - |P(\hat{Y}=1|Y=1,X_s=0)-P(\hat{Y}=1|Y=1,X_s=1)|
