...
class CompasDataset(AbstractDataset):
    ...
    def __init__(self, split, args, normalize=True, test_size=0.2, change_covariate=False):
        ...
        if test_size == 1.0:
            self.X_test, self.y_test, self.protected_test = features, labels, protected
            self.X_train, self.y_train, self.protected_train = features, labels, protected
            self.X_val, self.y_val, self.protected_val = features, labels, protected
        else:
            X_train, self.X_test, y_train, self.y_test, protected_train, self.protected_test = train_test_split(
                features, labels, protected, test_size=test_size, random_state=0
            )
            self.X_train, self.X_val, self.y_train, self.y_val, self.protected_train, self.protected_val = train_test_split(
                X_train, y_train, protected_train, test_size=test_size, random_state=0
            )
